...
<div class="card text-white bg-gradient-success mb-3">
				  ...
				  <span class=" text-success display-4" > Result  </span>					
					...
					// FIXME:
					$command = escapeshellcmd("python ML/fertilizer_recommendation/fertilizer_recommendation.py $Jsonn $Jsonp $Jsonk $Jsont $Jsonh $Jsonsm $Jsonsoil $Jsoncrop ");
					$output = shell_exec($command); // Capture the output of the Python script
					
					// Write user input and fertilizer name to CSV file
					$fp = fopen('ML/fertilizer_recommendation/fertilizer_recommendation.csv', 'a'); // Open CSV file in append mode
					$data = "$n,$p,$k,$t,$h,$sm,$soil,$crop,$output"; // Concatenate user input and fertilizer name
					fwrite($fp, $data . PHP_EOL); // Write data to CSV file
					fclose($fp); // Close CSV file
					
					// echo "Recommended Fertilizer is : $output"; // Display fertilizer name
